package devs2blu.loester.exer01.jogoDaVelha;
...
public class Tela extends JFrame implements ActionListener {
	...
	private void checarVencedorVertical() {
		for (int y = 0; y < 3; y++) {
			...
			int countJogador2 = 0;
			for (int x = 0; x < 3; x++) {
				if (botoes[x][y].jogador == 1)
					countJogador1++;
				else if (botoes[x][y].jogador == 2)
					countJogador2++;
			}
			if (countJogador1 == 3)
				terminarJogo(1);
			else if (countJogador2 == 3)
				terminarJogo(2);
		}
	}
	...
	private void checarVencedorDiagonal1() {
		...
		int countJogador2 = 0;
		for (int x = 0; x < 3; x++) {
			if (botoes[x][x].jogador == 1)
				countJogador1++;
			else if (botoes[x][x].jogador == 2)
				countJogador2++;
		}
		if (countJogador1 == 3)
			terminarJogo(1);
	...
	private void checarVencedorDiagonal2() {
		...
		int countJogador2 = 0;
		for (int x = 0; x < 3; x++) {
			if (botoes[x][2 - x].jogador == 1)
				countJogador1++;
			else if (botoes[x][2 - x].jogador == 2)
				countJogador2++;
		}
		if (countJogador1 == 3)
			terminarJogo(1);
